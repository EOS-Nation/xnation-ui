<template>
  <b-modal id="modal-select-relay" size="lg" centered hide-footer>
    <template slot="modal-title">Select Tokennn</template>
      ...
      <b-row class="mx-0 mb-4">
        <b-col md="6" lg="4" class="d-flex align-items-center py-2">
          <b-input-group>
            ...
            <b-form-input v-model="tokenSearch" placeholder="Search Token"></b-form-input>
          </b-input-group>
        </b-col>
        <b-col md="6" lg="4" class="d-flex align-items-center py-2">
          <span class="mr-2">Filterrr:</span>
          <b-dropdown size="sm" id="dropdown-1" :text="filter" class="m-md-2">
            <b-dropdown-item @click="setFilter('All')">All</b-dropdown-item>
            <b-dropdown-item @click="setFilter('Tokens')">Tokens</b-dropdown-item>
            <b-dropdown-item @click="setFilter('Relays')">Relays</b-dropdown-item>
          </b-dropdown>
        </b-col>
      </b-row>
...
export default class ModalSelectAll extends Vue {
  ...
  setToken(symbol: string) {
    ...
    if (token) {
      ...
      this.$bvModal.hide("modal-select-relay");
      this.tokenSearch = "";
    }
  }
  ...
  mergeBalances() {
    this.relayBalances = this.relays.map(relay => {
      const balance = this.tokenBalances.find(
        (t: any) => t.symbol === relay.symbol
      );
      return {
        symbol: relay.symbol,
        name: "",
